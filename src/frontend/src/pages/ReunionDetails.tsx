...
export default function ReunionDetails() {
  const { data: events = [], isLoading } = useGetAllEvents();

  const reunions = events.filter((e) => {
    const typeKey =
      typeof e.eventType === "object"
        ? Object.keys(e.eventType)[0]
        : String(e.eventType);
    return typeKey === "reunion";
  });

  return (
    <div className="pt-16">
      <HeroSection
        title="Reunion Meet Details"
        subtitle="Come together, reconnect, and celebrate the bonds that last a lifetime."
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-8">
            <h2 className="font-serif text-2xl font-bold text-foreground">
              {isLoading ? "Loading..." : `${reunions.length} Reunion Events`}
            </h2>
          </div>
          {isLoading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-muted-foreground">
                No reunion events scheduled at the moment.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
